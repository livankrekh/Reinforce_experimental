...
class PolicyNetwork(nn.Module):
	...
	def train(self, actions, states, rewards, new_states, batch_size=25, epochs=5, optim=torch.optim.Adam, lr=10**-4):
	...
	def loss_function(self, mean, std, actions, rewards, states, new_states):
		...
		total_rewards = self._rewards_policy(rewards, states, new_states, cumulative=False)

		loss = -torch.sum(torch.log(probs) * total_rewards.expand_as(probs))

		return loss

	def _rewards_policy(self, rewards, states, new_states, delta=1., coff=10, cumulative=True):
		total_rewards = []
		total = .0
		size = len(rewards)

		if not cumulative:
			return rewards.reshape((-1,1)) + torch.mean(rewards)
